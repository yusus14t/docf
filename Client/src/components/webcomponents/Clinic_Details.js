      ...
      <div className="">
        ...
        >
          ...
          <div className="d-flex flex-row  clinic-detail-img-container ">
            ...
            <div className="current-clicnic-token ml-5 d-flex flex-row" style={{ position:'relative'}}>
              <h1 style={{ position: 'absolute', left: '15%', top:'15%' }}>{clinicDetail?.doctors && (clinicDetail?.doctors["0"]?.token?.length === 1 ? `0${clinicDetail?.doctors["0"]?.token}` : clinicDetail?.doctors["0"]?.token) || '00'}</h1>
            </div>
          </div>
        </div>
            ...
            <div className="col-md-6 ">
              <div className="wating-area-clinic">
                <h4 className="text-center mb-3">Waiting List</h4>
                 ...
                 { waitingList?.length ? <ul className={`token-list $`}>
                     {waitingList.map( list => <li className=" p-2">
                      <div className="mt-auto">
                        <div className={`token-list-item d-flex flex-row justify-content-around ${ list?.token === (clinicDetail?.doctors && clinicDetail?.doctors[0]?.token) ? 'token-list-active' : '' }`}>
                          <div className="token ">
                            <h4 className="token-list-number">{ list?.token }</h4>
                          </div>
                          <div className="token-list-detail">
                            ...
                            <p className="mb-0 list-mobile-no">
                              Mobile Number : +91 { list?.phone ? `xxx-xxx-${ list?.phone.slice(-4) }` : '----------'}
                            </p>
          ...
          <div class="contact-details-clinic">
            ...
            <div class="sigma_info style-26 d-flex">
              ...
              <div class="sigma_info-description">
                <p>Call Us</p>
                <p class="secondary-color">
                  <b>{clinicDetail?.detail?.phone?.slice(0,3)}-{clinicDetail?.detail?.phone?.slice(4,7)}-{clinicDetail?.detail?.phone?.slice(-4)}</b>
                </p>
              </div>
            </div>
            <div class="sigma_info style-26 d-flex">
